...
def publicacao_texto():
    try:
        ...
        if not request.is_json:
            return jsonify({'erro': 'Dados enviados devem estar no formato JSON.'}), 400

        dados = request.get_json()
        texto = dados.get('conteudo')  # Certifique-se que a chave "conteudo" está correta

        # Valida se o texto não está vazio
        if not texto or not texto.strip():
            return jsonify({'erro': 'Texto não pode estar vazio ou apenas com espaços.'}), 400
        ...
        return jsonify({'mensagem': 'Postagem salva com sucesso!'}), 200
